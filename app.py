...
def random_quote():
    if not app.config['GLOBAL_VARIABLE']:
        now = datetime.now()
        ...
        return jsonify([
            {
                ...
                "quote": {
                    "author": "Yes",
                    "quote": "Will you go to Prom with me?"
                }
            },
            {
                "options": [
...
@app.route('/prom-fn2187', methods=['GET'])
def prom():
    app.config['GLOBAL_VARIABLE'] = not app.config['GLOBAL_VARIABLE']
    print(app.config['GLOBAL_VARIABLE'])
    return jsonify(app.config['GLOBAL_VARIABLE'])
